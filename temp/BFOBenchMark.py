...
class BFO_for_benchmark:
    def __init__(self,
        ...
        self.fitness_history = []  # 新增：用于记录每一代的最佳适应度
    ...
    def run(self):
        ...
        self.fitness_history = []  # 确保每次运行都重置历史记录
...
rastrigin_bfo = BFO_for_benchmark(
    **BFO_PARAMS,
    N=10,                             # 测试10维问题
    domain=(-100, 100),              # Rastrigin 函数的典型定义域
    fitness_function=sphere_function
)
best_pos_rastrigin, best_fit_rastrigin, history = rastrigin_bfo.run()
print(f"\nRastrigin 函数测试结果:")
...
try:
    ...
    print("无法设置中文字体 'SimHei'。如果您使用的是非 Windows 系统或者没有安装该字体，图表中的中文可能会显示为方块。")

# 绘制适应度曲线
plt.figure(figsize=(12, 6))
plt.plot(range(1, len(history) + 1), history, marker='o', linestyle='-', markersize=2)
plt.title("Rastrigin 函数的适应度曲线")
plt.xlabel("代数 (趋向操作次数 NC)")
plt.ylabel("全局最优适应度")
plt.grid(True)
plt.show()